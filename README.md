...
| Database | MongoDB via Mongoose 8, hosted on **MongoDB Atlas** — this is production (confirmed 2026-08-09 from the live container's env var); no separate dev/staging DB exists yet. **Urgent, decided plan:** Atlas becomes the permanent dev/staging DB; production moves to a new self-hosted Coolify Mongo — must land before onboarding more groups or existing groups accumulate more data. Not yet scheduled |
